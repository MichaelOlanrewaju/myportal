...
var f=a=="bottom"?"top":"bottom";var m=l=="right"?"left":"right";var c=document.getElementById("mfesecure-ts-style");if(!c){jQueryTs('<style id="mfesecure-ts-style" type="text/css">.trustedsite-body-noscroll{ overflow:hidden !important;position:fixed !important;}</style>').appendTo("head")
...
}if(TrustedSite.config.brand=="ts"){TrustedSite.animatetm(j)}document.body.appendChild(j);if(1){var k=TrustedSite.cookie_get("trustedsite_visit");
if(!k){TrustedSite.cookie_set("trustedsite_visit",1,24*60);TrustedSite.load_js("https://www.trustedsite.com/rpc/ajax?do=tmjs-visit&host="+n+"&rand="+new Date().getTime())
...
jQueryTs(a).css("bottom","-92px");window.setTimeout(function(){jQueryTs(a).css("bottom","0px")},10);TrustedSite.animatetm_hide(a,5000)
...
}if(!h){return}a=TrustedSiteInline.calc_wh(a,120,50);i="ts"}else{return}}}}}}}var b="https://cdn.ywxi.net/meter/"+f+"/"+d+"."+c;
...
var f=b&&b.length==2?b[1]:0;if(c&&c>0){return[c,parseInt(c/a*d)]}if(f&&f>0){return[parseInt(f/d*a),f]}return[a,d]},log:function(b,a){if(!a){return 0
